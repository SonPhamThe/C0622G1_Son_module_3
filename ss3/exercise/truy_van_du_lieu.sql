...
VALUES (1, 1, 8, 1),
       (1, 2, 10, 2),
       (2, 1, 12, 1); 
SELECT * FROM student s JOIN class c on s.class_id = c.class_id WHERE c.class_name = "A1";
SELECT * from student s JOIN mark m on s.student_id = m.student_id JOIN `subject` sub ON m.sub_id = sub.sub_id WHERE sub.sub_name = "CF";
